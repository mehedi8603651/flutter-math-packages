...
mixin SelectionManagerMixin<T extends StatefulWidget> on State<T>
  ...
  int getPositionForOffset(Offset globalOffset) {
    ...
    return target.basePosition + target.caretPositions[caretIndex];
  }
  ...
  TextSelection getWordRangeAtPoint(Offset globalOffset) {
    ...
    final extentCaretIndex = math.max(
      0,
      caretIndex + 1 >= target.caretPositions.length
          ? caretIndex - 1
          : caretIndex + 1,
    );
    final base = target.basePosition + target.caretPositions[caretIndex];
    final extent =
        target.basePosition + target.caretPositions[extentCaretIndex];
